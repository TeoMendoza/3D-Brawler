...
public class MagicianController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.W)) req.Velocity.Z += 0.5f;
        if (Input.GetKey(KeyCode.S)) req.Velocity.Z -= 0.5f;
        if (Input.GetKey(KeyCode.D)) req.Velocity.X += 0.25f;
        if (Input.GetKey(KeyCode.A)) req.Velocity.X -= 0.25f;
